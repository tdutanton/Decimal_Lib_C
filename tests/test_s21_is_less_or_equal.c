...
START_TEST(s21_less_or_equal_4) {
  ...
  compare_result check = CMP_TRUE;
  int res = s21_is_less_or_equal(first, max_decimal());
  ck_assert_int_eq(check, res);
}
END_TEST

START_TEST(s21_less_or_equal_5) {
  s21_decimal first = {{0xFFFFFFFF, 0, 0, 0x0}};
  compare_result check = CMP_TRUE;
  int res = s21_is_less_or_equal(first, max_decimal());
  ck_assert_int_eq(check, res);
}
...
Suite *test_s21_is_less_or_equal() {
  ...
  TCase *t_case = tcase_create("s21_is_less_or_equal");

  suite_add_tcase(suite, t_case);
  tcase_add_test(t_case, s21_less_or_equal_1);
  tcase_add_test(t_case, s21_less_or_equal_2);
  tcase_add_test(t_case, s21_less_or_equal_3);
  tcase_add_test(t_case, s21_less_or_equal_4);
  tcase_add_test(t_case, s21_less_or_equal_5);
  tcase_add_test(t_case, s21_less_or_equal_6);
  tcase_add_test(t_case, s21_less_or_equal_7);
  tcase_add_test(t_case, s21_less_or_equal_8);
  tcase_add_test(t_case, s21_less_or_equal_9);
  tcase_add_test(t_case, s21_less_or_equal_10);
  tcase_add_test(t_case, s21_less_or_equal_11);
  tcase_add_test(t_case, s21_less_or_equal_12);
  tcase_add_test(t_case, s21_less_or_equal_13);
